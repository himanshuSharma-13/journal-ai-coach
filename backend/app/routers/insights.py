...
async def create_insight(payload: InsightRequest, user: User = Depends(get_current_user), session: AsyncSession = Depends(get_session)) -> Insight:
    entries = (await session.scalars(select(JournalEntry).where(JournalEntry.user_id == user.id, JournalEntry.occurred_at >= payload.period_start, JournalEntry.occurred_at <= payload.period_end).order_by(JournalEntry.occurred_at.desc()).limit(30))).all()
